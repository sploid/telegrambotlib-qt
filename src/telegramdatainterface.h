...
public:
  static bool PathGet(const QJsonValue& data, const QString& path, T& target, bool show_warnings = true) {
    const QJsonObject object = show_warnings ? JsonHelper::PathGet(data, path).toJsonObject() : JsonHelper::jsonPathGetSilent(data, path).toJsonObject();
  ...
  static bool jsonPathGetArray(const QJsonValue& data, const QString& path, QList<T>& target, bool show_warnings = true) {
    QJsonArray array = show_warnings ? JsonHelper::PathGet(data, path).toJsonArray() : JsonHelper::jsonPathGetSilent(data, path).toJsonArray();
  ...
  static inline bool jsonPathGetArrayArray(const QJsonValue& data, const QString& path, QList<QList<T>>& target, bool show_warnings = true) {
    const QJsonArray array = show_warnings ? JsonHelper::PathGet(data, path).toJsonArray() : JsonHelper::jsonPathGetSilent(data, path).toJsonArray();
    if (array.isEmpty()) return false;
    for (auto itr = array.begin(); itr != array.end(); itr++) {
